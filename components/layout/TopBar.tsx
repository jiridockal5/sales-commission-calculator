...
function PlanNameInput({ plan }: { plan: CommissionPlan }) {
  ...
  return (
    <input
      ...
      className="h-8 w-full min-w-0 max-w-full rounded-md border border-transparent bg-transparent px-2 text-sm font-semibold text-slate-900 outline-none hover:border-slate-200 focus:border-brand-500 focus:bg-white sm:max-w-56"
    />
  );
}
...
export function TopBar({ plan, onOpenSidebar }: { plan: CommissionPlan; onOpenSidebar: () => void }) {
  ...
  return (
    <header className="no-print sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur">
      ...
      <button type="button" className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-md text-slate-600 hover:bg-slate-100 lg:hidden" onClick={onOpenSidebar} aria-label="Open menu">
        ...
        <Menu
          ...
          trigger={({ toggle, open }) => (
            <button
              ...
              className={cn("inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-md text-slate-500 hover:bg-slate-100 xl:h-8 xl:w-auto xl:gap-1 xl:px-1.5", open && "bg-slate-100")}
            >
      ...
      <div className="ml-auto flex shrink-0 items-center gap-1">
        ...
        <Button className="xl:hidden" onClick={() => store.saveNow()} title={saveLabel} aria-label={saveLabel}>
          {store.saveStatus === "saving" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Save className="h-3.5 w-3.5" />}
        </Button>
        ...
        <Menu
          trigger={({ toggle }) => (
            <Button variant="ghost" onClick={toggle} aria-label="More actions" className="w-11 px-0 xl:w-auto xl:px-2">
      ...
      <div className="flex gap-2 border-t border-slate-100 px-3 py-2 xl:hidden">
        <Select<CurrencyCode>
          ariaLabel="Currency"
